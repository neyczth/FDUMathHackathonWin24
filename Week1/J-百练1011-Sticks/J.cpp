...
bool backtracking(const int &num_corr_stick, const int &corr_length, const int &start_idx) {
    if (num_corr_stick >= sum_length / trial_length) return true;

    bool is_ok = 0;
    int &&rest_length = trial_length - corr_length;
    int last_trial = -1;
    for (int i = start_idx; i < num_sticks; i++) {
        if (!vst[i] && sticks[i] <= rest_length && sticks[i] != last_trial) {
            vst[i] = true; last_trial = sticks[i];
            if (rest_length == sticks[i]) is_ok |= backtracking(num_corr_stick+1, 0, 0);
            else is_ok |= backtracking(num_corr_stick, corr_length + sticks[i], i+1);
